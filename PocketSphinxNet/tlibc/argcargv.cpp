...
//NOTICE:BIG_TODO GetCommandLine()
		;
	int szSysCmd = 0
//NOTICE:BIG_TODO lstrlen(sysCmd)
		;

	// copy the system command line
	TCHAR *cmd = 0
		//NOTICE:BIG_TODO (TCHAR*)HeapAlloc(GetProcessHeap(), 0, sizeof(TCHAR)*(szSysCmd+1))
		;
	_rawCmd = cmd;
	if (!cmd)
		return 0;
	//NOTICE:BIG_TODO lstrcpy(cmd, sysCmd);
...
void _term_args()
{
	if (_rawCmd)
	{
		//NOTICE:BIG_TODO HeapFree(GetProcessHeap(), 0, _rawCmd);
	}
}