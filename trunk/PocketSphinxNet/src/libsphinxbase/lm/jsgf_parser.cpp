...
#define YYTRANSLATE(YYX)						\
	((unsigned int) (YYX) <= YYMAXUTOK ? get_yytranslate()[YYX] : YYUNDEFTOK)

/* YYTRANSLATE[YYLEX] -- Bison symbol number corresponding to YYLEX.  */

static const yytype_uint8* get_yytranslate()
{
...
static const char *const * get_yytname()
{
...
static const yytype_uint8* get_yyr1()
{
...
static const yytype_uint8* get_yyr2()
{
...
static const yytype_uint8* get_yydefact()
{
...
/* YYDEFGOTO[NTERM-NUM].  */
static const yytype_int8* get_yydefgoto()
{
...
static const yytype_int8* get_yypact()
{
...
/* YYPGOTO[NTERM-NUM].  */
static const yytype_int8* get_yypgoto()
{
...
static const yytype_uint8* get_yytable()
{
...
static const yytype_int8* get_yycheck()
{
...
static const yytype_uint8* get_yystos()
{
	...
	yysyntax_error (char *yyresult, int yystate, int yychar)
{
	...
	else
	{
		...
		YYSIZE_T yysize0 = yytnamerr (0, get_yytname()[yytype]);
...
# endif
		...
		yyarg[0] = get_yytname()[yytype];
		yyfmt = yystpcpy (yyformat, yyunexpected);

		for (yyx = yyxbegin; yyx < yyxend; ++yyx)
			if (get_yycheck()[yyx + yyn] == yyx && yyx != YYTERROR)
			{
				...
				yyarg[yycount++] = get_yytname()[yyx];
				yysize1 = yysize + yytnamerr (0, get_yytname()[yyx]);
...
yybackup:
	...
	yyn += yytoken;
	if (yyn < 0 || YYLAST < yyn || get_yycheck()[yyn] != yytoken)
		goto yydefault;
	yyn = get_yytable()[yyn];
	if (yyn <= 0)
	{
...
#line 121 "jsgf_parser.y"
		{ (yyval.rhs) = (jsgf_rhs_t*)ckd_calloc(1, sizeof(*(yyval.rhs)));
...
#line 136 "jsgf_parser.y"
		{ (yyval.rule) = jsgf_define_rule(jsgf, 0, (yyvsp[(2) - (3)].rhs), 0); }
		break;
...
#line 1580 "jsgf_parser_cpp"
	...
	*++yyvsp = yyval;
	...
	yystate = get_yypgoto()[yyn - YYNTOKENS] + *yyssp;
	if (0 <= yystate && yystate <= YYLAST && get_yycheck()[yystate] == *yyssp)
		yystate = get_yytable()[yystate];
	else
		yystate = get_yydefgoto()[yyn - YYNTOKENS];
...
yyerrlab1:
	...
	for (;;)
	{
		...
		{
			yyn += YYTERROR;
			if (0 <= yyn && yyn <= YYLAST && get_yycheck()[yyn] == YYTERROR)
			{
				yyn = get_yytable()[yyn];
				if (0 < yyn)
					break;
		...
		yydestruct ("Error: popping",
			get_yystos()[yystate], yyvsp, yyscanner, jsgf);
		YYPOPSTACK (1);
		yystate = *yyssp;
		YY_STACK_PRINT (yyss, yyssp);
	}
...
yyreturn:
	...
	while (yyssp != yyss)
	{
		yydestruct ("Cleanup: popping",
			get_yystos()[*yyssp], yyvsp, yyscanner, jsgf);
		YYPOPSTACK (1);
	}
#ifndef yyoverflow
	if (yyss != yyssa)
